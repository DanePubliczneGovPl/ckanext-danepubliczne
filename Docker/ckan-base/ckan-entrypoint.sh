...
ckan-paster --plugin=ckan datastore set-permissions | sudo -u postgres psql --set ON_ERROR_STOP=1
ckan-paster --plugin=ckan db init -c "${CKAN_CONFIG}/ckan.ini"
ckan-paster --plugin=ckanext-archiver archiver init -c "${CKAN_CONFIG}/ckan.ini"
ckan-paster --plugin=ckanext-qa qa init -c "${CKAN_CONFIG}/ckan.ini"
ckan-paster --plugin=ckanext-report report initdb -c "${CKAN_CONFIG}/ckan.ini"
